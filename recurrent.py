...
class Highway_SimpleRNN(Recurrent):
    ...
    def build(self):
        ...
        self.U_w = self.inner_init((self.output_dim, self.output_dim),
                                 name='{}_U_w'.format(self.name))
        self.b_w = K.zeros((self.output_dim,), name='{}_b_w'.format(self.name))
    ...
    def step(self, h, states):
        ...
        if len(states) == 2:
            ...
            B_U = [1., 1., 1.]
        output = self.activation(h + K.dot(prev_output * B_U[0], self.U))
        x_w = self.inner_activation(h + K.dot(prev_output * B_U[0], self.U))
        # still needs work
        return output, [output]
...
class Residual_LSTM(Recurrent):
    ...
    def __init__(self, output_dim,
        ...
        self.b_regularizer = regularizers.get(b_regularizer)
        self.dropout_W, self.dropout_U = dropout_W, dropout_U
        super(Residual_LSTM, self).__init__(**kwargs)
    ...
    def get_config(self):
        config = {"output_dim": self.output_dim,
                  ...
                  "dropout_W": self.dropout_W,
                  "dropout_U": self.dropout_U}
        base_config = super(Residual_LSTM, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

class Residual_GRU(Recurrent):
    '''Gated Recurrent Unit - Cho et al. 2014.
    ...
    def __init__(self, output_dim,
        ...
        self.b_regularizer = regularizers.get(b_regularizer)
        self.dropout_W, self.dropout_U = dropout_W, dropout_U
        super(Residual_GRU, self).__init__(**kwargs)
    ...
    def preprocess_input(self, x, train=False):
        ...
        x_h = time_distributed_dense(x, self.W_h, self.b_h, dropout,
                                     input_dim, self.output_dim, timesteps)
        return K.concatenate([x_z, x_r, x_h, x], axis=2)
    ...
    def step(self, x, states):
        ...
        x_r = x[:, self.output_dim: 2 * self.output_dim]
        x_h = x[:, 2 * self.output_dim: 3 * self.output_dim]
        x_0 = x[:, 3 * self.output_dim:]

        z = self.inner_activation(x_z + K.dot(h_tm1 * B_U[0], self.U_z))
        r = self.inner_activation(x_r + K.dot(h_tm1 * B_U[1], self.U_r))

        hh = self.activation(x_h + K.dot(r * h_tm1 * B_U[2], self.U_h))
        h =  x_0 + (z * h_tm1 + (1 - z) * hh)
        return h, [h]
    ...
    def get_config(self):
        config = {"output_dim": self.output_dim,
                  ...
                  "dropout_W": self.dropout_W,
                  "dropout_U": self.dropout_U}
        base_config = super(Residual_GRU, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

class Residual_SimpleRNN(Recurrent):
    '''Fully-connected RNN where the output is to be fed back to input.
